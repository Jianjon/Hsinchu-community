...
async function main() {
    const cityPath = path.join(LOCAL_DB_BASE, TARGET_CITY);
    if (!fs.existsSync(cityPath)) {
        console.error(`City path not found: ${cityPath}`);
        return;
    }

    const districts = fs.readdirSync(cityPath).filter(d => !d.startsWith('.'));
    console.log(`🚀 Starting county-wide diversification for Hsinchu County (${districts.length} districts)...`);

    for (const district of districts) {
        const districtPath = path.join(cityPath, district);
        const villages = fs.readdirSync(districtPath).filter(v => !v.startsWith('.'));

        console.log(`\n📂 Processing district: [${district}] (${villages.length} villages)`);
        ...
        for (const village of villages) {
            ...
            // Check if already processed (h fields and they aren't empty)
            if (wikiData.intro_geo && wikiData.intro_history && wikiData.intro_geo.length > 50) {
                // If it's already there, we skip it to save tokens, 
                // UNLESS we want to force re-generation (e.g. for Zhubei cleanup if needed)
                if (district !== '竹北市') { // Assuming Zhubei is done, skip it
                    // console.log(`⏩ [${village}] already h-segment content. Skipping.`);
                    continue;
                }
            }

            console.log(`📝 [${district}][${village}] generating content...`);

            try {
                const prompt = PROMPT_TEMPLATE(TARGET_CITY, district, village, wikiData);
                const aiResult = await callGemini(prompt);

                wikiData.intro_geo = aiResult.intro_geo;
                wikiData.intro_history = aiResult.intro_history;
                wikiData.features = aiResult.features || wikiData.features;
                ...
                // If we hit a serious error (like 429), we might want to wait longer or stop
    ...
    console.log("\n🎉 County-wide two-segment diversification complete.");
}
